...
def read_obj_file(file_path):
    vertices = []
    faces = []  # Initialize a list to store face information
    with open(file_path, 'r') as file:
        for line in file:
            if line.startswith('v '):
                ...
                # OBJ file uses 1-based indexing, so we subtract 1 to convert to 0-based indexing
                face = [int(v1) - 1, int(v2) - 1, int(v3) - 1]
                faces.append(face)

    return np.array(vertices), np.array(faces)

# TODO for Avatar
def dense_obj_file(vertices, faces, dense):
    print(f"==> doing dense operation: {dense}")
    if dense == 1:
        return vertices, faces

    n_faces = len(faces)
    new_vertices = np.zeros((n_faces, 3))
    new_faces = np.zeros((n_faces*4, 3), dtype=np.int64)

    for i, face in enumerate(faces):
        new_xyz = np.sum(vertices[face], axis=0)/3.0
        new_idx = len(vertices) + i

        new_vertices[i] = new_xyz
        new_faces[i*4] = face
        new_faces[i*4 + 1] = [face[0], face[1], new_idx]
        new_faces[i*4 + 2] = [face[0], new_idx, face[2]]
        new_faces[i*4 + 3] = [new_idx, face[1], face[2]]

    vertices = np.concatenate([vertices, new_vertices], axis=0)
    faces = new_faces

    return dense_obj_file(vertices, faces, dense-1)
...
def fetchPly(path):
    ...
    # colors = np.vstack([vertices['red'], vertices['green'], vertices['blue']]) / 255.0
...
def readCircleCamInfo(path,opt):
    ...
    if not os.path.exists(ply_path):
        ...
        # TODO: 加入SMPL模型
        elif opt.init_shape == 'SMPL':
            obj_file = "smpl_obj/A_shape_3.obj"
            print(f"==> use obj file {obj_file}")
            ForCondition = True
            if ForCondition:
                dense_obj = 5
            else:
                dense_obj = 1
            xyz, face = read_obj_file(obj_file)
            print(f"==> before dense operation: {len(xyz)}")
            xyz, face = dense_obj_file(xyz, face, dense=dense_obj)
            print(f"==> after dense operation: {len(xyz)}")
            vertexs = xyz

            num_pts = xyz.shape[0]
            rotation_angle_z = np.pi * 0
            rotation_angle_y = np.pi
            rotation_angle_x =  - np.pi / 2
            point_cloud_center = np.mean(xyz, axis=0)
            xyz = xyz-point_cloud_center
            ...
            rotated_point_cloud = np.dot(np.dot(np.dot(xyz, rotation_matrix_x.T), rotation_matrix_y.T), rotation_matrix_z.T)
            rotated_point_cloud = rotated_point_cloud + np.array([0, 0, 0.2])
            xyz = np.array(rotated_point_cloud)
        else:
